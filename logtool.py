...
#获取命令行
try:
    opts,args = getopt.getopt(sys.argv[1:],"m:t:a:l:f:h",["help","start=","server=","end="])
...
if f_opt:
    ...
    filename = module_name.lower() + '.' + datetime.now().strftime('%Y-%m-%d %H:%M:%S')
